...
public interface SubjectRepository extends JpaRepository<Subject, String>{
    ...
    @Query(value = "SELECT s.subid,s.name,t.name FROM student_subjects ss inner join subject s on ss.subjects_subid=s.subid and ss.department=s.department inner join student st on ss.student_sid=st.sid inner join teacher_subjects ts on ss.subjects_subid=ts.subjects_subid and ss.department=ts.department inner join teacher t on ts.teacher_tid=t.tid  WHERE student_sid = :studentId", nativeQuery = true)
